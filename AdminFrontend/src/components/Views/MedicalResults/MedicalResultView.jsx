...
export const MedicalResultView = ({ appointmentId, currentDoctorId, onBack }) => {
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                setPrefilledData({
                    ...
                    doctorAccountId: doctor.accountId || doctor.AccountId || doctor.userId || doctor.UserId, 
    ...
    console.log("=== ПРОВЕРКА ПРАВ ДОКТОРА ===");
    console.log("1. Твой ID из пропсов (currentDoctorId):", currentDoctorId);
    console.log("2. ID доктора из приема (Profile ID):", prefilledData?.doctorId);
    console.log("3. Account ID доктора из БД (doctorAccountId):", prefilledData?.doctorAccountId);
    console.log("===============================");

    const isOwnerDoctor = 
        String(currentDoctorId) === String(prefilledData?.doctorId) || 
        String(currentDoctorId).toLowerCase() === String(prefilledData?.doctorAccountId).toLowerCase();
